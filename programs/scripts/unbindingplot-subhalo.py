...
outputfilename = "unbinding_plot_subhalo_"+str(halo)+'_3D'
...
def get_clump_data(halo,noutput):
    ...
    while(i<data_int.shape[0]):
        if (data_int[i,2] in children or data_int[i,2]==halo or data_int[i,0]==halo): #if parent is halo or one of the children
...
if __name__ == "__main__":
    ...
    fig.suptitle('unbinding plot 3D halo '+str(halo), family='serif', size=10)
    ...
    # PLOT HALOS
    m.plot_3D(halox,haloy,haloz,ax1,ax2,ax3,ax4,pointsize,pointalpha,'subhalo',0,halo) 

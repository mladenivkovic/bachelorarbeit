...
clumpid=str(argv[1])
noutput=len(argv)-2
outputfilename = "cmp_plot_all_log"+clumpid
title='cumulative mass profile plot clump '+clumpid
workdir= str(getcwd())
...
if __name__ == "__main__":
    ...
    for i in range(noutput):
        ...
        bins=bins[:5]
        bins=bins.lstrip("0")
        ax1.loglog(dist,phi,label=pot+' potential with '+bins+' '+style)
    ...
    ax1.set_ylabel('cumulative mass', size=20)
